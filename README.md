...
One HTML file plus one plain JS file — no build step, no backend, no dependencies. Forecast data comes from [Open-Meteo](https://open-meteo.com) (free, no API key) and map outlines from [Natural Earth](https://www.naturalearthdata.com); everything else runs in your browser. It also works opened straight from a local file.

Hourly forecasts cover ~16 days ahead; the app shows a confidence note based on lead time.

## Development

The app is `index.html` (markup + CSS) plus `app.js` (all logic) — still no build step; `app.js` ends with a CommonJS export block that browsers ignore, so tests `require()` it directly. Tests use Node's built-in runner (no dependencies):

```sh
npm test   # = node --test
```

Tests cover the route model, both timing modes, weather interpolation (including midnight rollover and circular wind direction), the fueling planner, and localStorage persistence. Rendering functions are exercised in the browser only.
